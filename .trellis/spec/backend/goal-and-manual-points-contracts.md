...
- Core configuration uses optimistic `revision` and is editable only before any assignment of the shared definition is terminal. A student may edit only their own pending proposal; the responsible parent may edit active configuration. After evaluation, only the responsible parent may append immutable `goal_notes`; terminal facts are never overwritten.
...
| Goal is not active or already terminal | `STATE_CONFLICT` |
| Core edit revision is stale, or any shared assignment is terminal | `STATE_CONFLICT` |
...
- Good: student proposes, one linked parent approves and becomes responsible, that parent evaluates success, and exactly one reward ledger entry updates the balance.
- Base: parent-personal goal reaches a terminal state with zero points and no gift; it creates no student ledger entry.
- Bad: reading the balance and later inserting a penalty without a shared row lock permits concurrent overdraft.

## 6. Tests Required

- Integration: two linked parents; first approves; second cannot evaluate; first succeeds; fresh goal read contains responsible parent and actual reward.
...
Update `point_balance_projection` directly, let any linked parent edit a terminal goal, or delete a mistaken penalty.
