...
export const planBindings = pgTable(
  "plan_bindings",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    libraryId: uuid("library_id")
      .notNull()
      .references(() => planLibrary.id),
...
export const planActivations = pgTable(
  ...
  (t) => [
    unique("plan_activations_execution_unique").on(t.executionPlanId),
    check(
      "plan_activations_range_check",
...
/** Mutable per-occurrence execution state. Plan entries and facts remain immutable snapshots. */
export const scheduleTaskExecutions = pgTable(
  "schedule_task_executions",
  {
    scheduleItemId: uuid("schedule_item_id")
      .primaryKey()
      .references(() => scheduleItems.id),
    taskType: text("task_type").notNull().default("normal"),
    checklist: jsonb("checklist").$type<Array<Record<string, unknown>>>().notNull().default([]),
    pomodoro: jsonb("pomodoro").$type<Record<string, unknown>>().notNull().default({}),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => [
    check(
      "schedule_task_executions_type_check",
      sql`${t.taskType} IN ('normal', 'homework', 'exercise')`,
    ),
  ],
);

export const planLibraryCommands = pgTable(
  "plan_library_commands",
  {
    id: uuid("id").primaryKey().defaultRandom(),
