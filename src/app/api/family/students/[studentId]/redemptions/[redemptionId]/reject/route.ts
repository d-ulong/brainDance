...
import { rejectRedemptionBodySchema } from "@/app/api/_lib/m6-schemas";
...
import { rejectRedemptionRequest } from "@/modules/redemption/redemption.service";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const body = rejectRedemptionBodySchema.parse(await request.json());

    const result = await rejectRedemptionRequest(db, {
      parentId: dbUser.id,
      studentId,
      redemptionId,
      reason: body.reason,
