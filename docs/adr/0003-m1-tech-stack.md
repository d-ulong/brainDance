...
`docs/architecture.md` 已方向性选择 TypeScript 全栈 Web + PostgreSQL + SQL 迁移 + 经验证会话库，但未锁定具体库。M1 需要最小闭环，同时不阻碍 M2–M3 的事务 outbox、Module 边界与授权 epoch。ADR-0001 已定大陆单区域托管 PostgreSQL，本 ADR 只选应用层栈。
